  ...
  if (compact) {
    ...
    return (
      <div className="monthly-filters">
        <div className="monthly-filter-primary">
          ...
          <button
            ...
            aria-controls={`monthly-advanced-fields-${uniqueId}`}
            onClick={() => setIsExpanded(!isExpanded)}
          >
            <Filter size={16} />
            <span>{isEn ? 'More filters' : 'Más filtros'}{advancedFilterCount ? ` (${advancedFilterCount})` : ''}</span>
            <span aria-hidden="true">{isExpanded ? '−' : '+'}</span>
          </button>
        </div>

        <div id={`monthly-advanced-fields-${uniqueId}`} className="monthly-filter-advanced" data-expanded={isExpanded}>
          ...
          {showClear && hasNonDateFilters && <button type="button" className="monthly-clear" onClick={clearMonthlyFilters}><X size={15} />{isEn ? 'Clear filters' : 'Limpiar filtros'}</button>}
        </div>
      </div>
    );
  }
