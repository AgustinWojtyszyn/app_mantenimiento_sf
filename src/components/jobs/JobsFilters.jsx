...
export default function JobsFilters({
  ...
  return (
    <div className={compact ? "dashboard-filters" : "bg-white dark:bg-slate-900 p-4 md:p-5 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800"}>
      {compact && <button type="button" className="dashboard-filter-toggle" aria-expanded={expanded} aria-controls="daily-filter-fields" onClick={() => setExpanded(!expanded)}>{isEn ? 'Filters' : 'Filtros'} · {date}{searchTerm || requestedBy || selectedLocation !== 'all' || selectedStatus !== 'all' ? (isEn ? ' · Active' : ' · Activos') : ''}<span>{expanded ? '−' : '+'}</span></button>}
      <div id={compact ? 'daily-filter-fields' : undefined} data-expanded={expanded} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-[12rem_minmax(0,1fr)_18rem_13rem_16rem_12rem] gap-3 items-end">
